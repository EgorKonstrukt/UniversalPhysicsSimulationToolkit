...
class ToolManager:
    ...
    def create_tool_buttons(self):
        ...
        spawn_panel = UIPanel(relative_rect=pygame.Rect(5, 5, 200, 640),
                              manager=self.ui_manager.manager)

        section_y = 0

        def add_section_label(text, y):
            UILabel(relative_rect=pygame.Rect(0, y, 190, 25),
                    text=f"-- {text} --",
                    manager=self.ui_manager.manager,
                    container=spawn_panel)
            return y + 30

        def add_tool_button(y, name, icon_path):
            button = UIButton(
                relative_rect=pygame.Rect(10, y, 120, 45),
                text=name,
                manager=self.ui_manager.manager,
                container=spawn_panel
            )
            UIImage(
                relative_rect=pygame.Rect(135, y + 2, 40, 40),
    ...
    def _register_tools(self):
        ...
        spawn_tools = {
            "Circle": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/circle.png",
                "window": self.ui_manager.window_circle,
                "spawner": self.spawner.spawn_circle,
                "drag_spawner": self.spawner.spawn_circle_dragged
            },
            "Rectangle": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/rectangle.png",
                "window": self.ui_manager.window_rectangle,
                "spawner": self.spawner.spawn_rectangle,
                "drag_spawner": self.spawner.spawn_rectangle_dragged
            },
            "Triangle": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/triangle.png",
                "window": self.ui_manager.window_triangle,
                "spawner": self.spawner.spawn_triangle,
                "drag_spawner": self.spawner.spawn_triangle_dragged
            },
            "Polyhedron": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/polyhedron.png",
                "window": self.ui_manager.window_polyhedron,
                "spawner": self.spawner.spawn_polyhedron,
                "drag_spawner": self.spawner.spawn_polyhedron_dragged
            },
            "Spam": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/spam.png",
                "spawner": self.spawner.spawn_spam,
                "drag_spawner": None
            },
            "Human": {
                "type": "spawn",
                "icon": "sprites/gui/spawn/human.png",
                "spawner": self.spawner.spawn_human,
                "drag_spawner": None
            }
        }
        constraint_tools = {
            "Spring": {
                "type": "constraint",
                "icon": "sprites/gui/tools/spring.png",
                "handler": self.create_spring
            },
            "Pivot": {
                "type": "constraint",
                "icon": "sprites/gui/tools/pivot.png",
                "handler": self.create_pivot
            },
            "Rigid": {
                "type": "constraint",
                "icon": "sprites/gui/tools/rigid.png",
                "handler": self.create_rigid
            }
        }
        self.tools["Explosion"] = {
            "type": "tool",
            "icon": "sprites/gui/tools/explosion.png",
            "handler": self.trigger_explosion
        }
        self.tools["Drag"] = {
            "type": "tool",
            "icon": "sprites/gui/tools/explosion.png",
    ...
    def handle_tool_button_press(self, button, game_app):
        """Обработка нажатия на кнопку инструмента"""
    ...
    def create_rigid(self, start_body, end_body):
        """Создание жесткого соединения между телами"""
        if start_body and end_body:
            self.physics_manager.space.add(
                pymunk.PinJoint(start_body, end_body, (0, 0), (0, 0))
            )

    def trigger_explosion(self, position):
        """Создание взрыва в указанной точке"""
        print("boom!")
