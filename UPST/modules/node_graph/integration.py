...
def register_context_menu(plugin_manager):
    ...
    def get_items(pm, obj, world_pos=None):
        ...
        elif obj and hasattr(obj, 'id') and obj.id in ngm.active_graph.nodes:
            target_node = ngm.active_graph.nodes[obj.id]

        if target_node:
            has_custom = False
            if hasattr(target_node, 'get_context_menu_items'):
                try:
                    custom_items = target_node.get_context_menu_items(ngm)
                    if custom_items:
                        items.extend(custom_items)
                        has_custom = True
                except Exception as e:
                    Debug.log_error(f"Error getting custom menu in integration: {e}", "NodeGraph")

            if not has_custom:
                items.append(ConfigOption(f"Delete Node '{target_node.name}'",
                                          handler=lambda cm: ngm.delete_node(target_node.id),
                                          icon="sprites/gui/erase.png"))
                items.append(ConfigOption("---", handler=lambda cm: None))
                items.append(ConfigOption("Disconnect All", handler=lambda cm: ngm._disconnect_all_node(target_node.id),
                                          icon="sprites/gui/disconnect.png"))
        else:
            if world_pos:
                items.append(ConfigOption("Create Node Here", handler=lambda cm: ngm._open_spawn_menu(world_pos),
                                          icon="sprites/gui/add.png"))
            items.append(ConfigOption("Create New Graph", handler=lambda cm: ngm.create_graph(),
