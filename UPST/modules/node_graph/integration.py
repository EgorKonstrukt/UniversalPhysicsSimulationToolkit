...
def register_context_menu(plugin_manager):
    ...
    def get_items(pm, obj, world_pos=None):
        ...
        if target_node:
            ...
            items.append(ConfigOption("Disconnect All",
                                      handler=lambda cm: ngm._disconnect_all_node(target_node.id),
                                      icon="sprites/gui/disconnect.png"))

            from UPST.modules.node_graph.node_types import OscillatorNode, ToggleNode
            if isinstance(target_node, OscillatorNode):
                items.append(ConfigOption("---", handler=lambda cm: None))
                items.append(ConfigOption(f"Toggle Power ({'ON' if target_node.enabled else 'OFF'})",
                                          handler=lambda cm: ngm._toggle_oscillator(target_node)))
        ...
        if obj is None and world_pos and ngm.active_graph:
            items.append(ConfigOption("Create Node Here",
                                      handler=lambda cm: ngm._open_spawn_menu(world_pos),
                                      icon="sprites/gui/add.png"))
            items.append(ConfigOption("Create New Graph",
                                      handler=lambda cm: ngm.create_graph(),
