...
class GizmosDemo:
    ...
    def __init__(self, camera):
        ...
        self._demos = {
            ...
            "step_gray_scott": {
                "title": "step_gray_scott",
                "description": "",
                "function": self.step_gray_scott
            },
            "step_scalar_phi4": {
            ...
            "step_ising_metropolis": {
                "title": "step_ising_metropolis",
                "description": "",
                "function": self.step_ising_metropolis
            },
            "step_nbody_3d_perspective": {
                "title": "step_nbody_3d_perspective",
                "description": "",
                "function": self.step_nbody_3d_perspective
            },
            "demo_fluid_dynamics": {
    ...
    def step_scalar_phi4(self, position=(0, 0)):
        ...
        Gizmos.draw_text(
            (cx - halfx + 1200, cy - halfy + 200),
            """Что это такое (в терминах этой модели)
        Это численная 2D-реализация скалярного поля φ с нелинейным φ^4-взаимодействием, дискретизированная
         на регулярной квадратной решётке. Модель описывает односкалярное поле φ(x,y,t) с уравнением движения (в безразмерных единицах):
            ∂_t^2 φ = Δφ - m^2 φ - λ φ^3
        Здесь Δ — лапласиан на решётке, m^2 — массовой параметр (величина, задающая линейную часть силы),
         а λ — коэффициент нелинейного взаимодействия φ^4.

        Зачем это нужно (цели и применения)
        • Педагогика и интуиция: простая, но богатая модель для изучения волновой динамики, нелинейных
         эффектов, солитонов, флуктуаций и процессов релаксации/тепловизации.  
        • Численные эксперименты: тестирование интеграторов (symplectic leapfrog), контроля энергии и
         устойчивости.  
        • Исследования нелинейных волн и фазовых переходов: φ^4—модель — канонический пример теоретической
         и вычислительной физики.  
        • Визуализация и отладка: даёт наглядные карты поля, градиентов и локальной плотности энергии.

        Ключевые элементы модели и их роль (с точки зрения реализации)
        1. Решётка (Nx×Ny, шаг a): дискретизует непрерывное поле в узлах. Размеры и шаг задают
         пространственное разрешение и спектр допустимых волн.  
        2. Поле φ и канонический импульс π: хранится φ(t) и π(t±dt/2) для leapfrog'а — это
         симплектическая схема, сохраняющая геометрию фазового пространства лучше, чем обычные схемы.  
        3. Лапласиан (конечные разности): реализует ∇^2φ через суммы соседних узлов (стандартная
         пятиточечная схема).  
        4. Нелинейная сила −λ φ^3 и линейная −m^2 φ: дают совместно локальную силу
         f(φ) = −m^2 φ − λ φ^3, которая делает систему нелинейной и потенциально богатой по динамике.  
        5. Периодические граничные условия (PBC): делают область топологически тором, избавляя от
         краевых эффектов и упрощающая спектральный анализ.  
        6. Интегратор leapfrog: обновляет π на полушаг, затем φ на полный шаг. Это симплектический
         метод второго порядка, подходящий для гамильтоновых систем — хорошо сохраняет энергию в долгосрочной эволюции.  
        7. Мониторинг энергии H: вычисление локальной плотности энергии и её суммы служит контрольной
         метрикой — если энергия «прыгает», значит интегратор плохо настроен или нарушено
          условие устойчивости.

        Как это работает пошагово (алгоритм для одного шага)
        0. Инициализация (единожды): заданы Nx, Ny, a, m2, lam, dt; φ заполняется маленьким
         «горбом» + шум; π инициализируются (обычно нулём). Проводится проверка устойчивости:
          dt ≤ a/√d (здесь d=2).  
        1. Для каждой ячейки вычисляем лапласиан Δφ через соседей (с учётом PBC).  
        2. Считаем силу force = Δφ − m^2 φ − λ φ^3. Это правая часть уравнения ∂_t^2 φ = force.  
        3. Leapfrog: π(t+dt/2) = π(t−dt/2) + dt * force. Затем φ(t+dt) = φ(t) + dt * π(t+dt/2).
         Таким образом шаг обновления разбит на два простых линейных обновления.  
        4. Обновляем системные переменные и увеличиваем время t += dt.  
        5. Считаем энергию на сетке: плотность = 1/2 π^2 + 1/2 (∇φ)^2 + 1/2 m^2 φ^2 + (λ/4) φ^4;
         сумма по узлам даёт контроль на сохранность H.  
        6. Визуализация: тепловая карта по φ (с обрезкой по φ_clip для читаемости), стрелки
        градиента ∇φ периодически (чтобы не засорять), HUD с метриками (t, E, dt, параметры).

        Практические замечания и ограничения (что важно знать)
        • Устойчивость: для явных схем (включая leapfrog) dt ограничен волновой скоростью и
         сеткой — нарушение приводит к нестабильности. Проверка dt ≤ a/√d критична.  
        • Дискретизационная погрешность: лапласиан с конечными разностями аппроксимирует ∇^2 лишь
         с ошибкой O(a^2). Для изучения мелкомасштабных эффектов нужно уменьшать a и увеличивать Nx×Ny.  
        • Нелинейность и стягивание энергии: при больших λ решение может развивать быстрые изменения
         (шоки в дискретной версии), требуется аккуратность с dt.  
        • Шум в начальной конфигурации и рампа: маленькие случайные возмущения запускают динамику
         и предотвращают застревание в тривиальных решениях.  
        • PBC: убирают краевые отражения, но искажают физику если вы хотите изучать эффекты
         с открытыми краями.  
        • Отслеживание энергии: важнейший диагностический инструмент — симплектический интегратор
         стремится сохранять энергию, но накопленные численные ошибки проявляются в медленном дрейфе.

        Параметры визуализации (почему именно так)
        • phi_clip: явное ограничение по цветовой шкале делает картинку информативной — исключает
         доминирование нескольких пиков над общей структурой.  
        • Редкие стрелки градиента: показывают направление и локальную «течётность» поля,
         не загромождая сцену.  
        • HUD с E и t: быстрый контроль корректности и режима (стационарность, периодичность, хаотизация).

        Интерпретация результатов (чего ожидать)
        • Малые возмущения могут распространяться как волновые пакеты, отражаться/интерферировать
         (с PBC — проходить сквозь границу).  
        • При сильной нелинейности могут образовываться локализованные структуры (псевдосолитоны),
         высокая локальная энергия и быстрый обмен между модами.  
        • Если интегратор и параметры корректны, суммарная энергия должна оставаться почти постоянной
         во времени — это признак корректной симуляции.

        Коротко: модель — это компактный и наглядный «песочница»-симулятор нелинейного скалярного поля.
         Она полезна для обучения, тестирования численных схем, изучения динамики волн и поведения
          нелинейных систем. Реализация через leapfrog + PBC + визуализация даёт баланс простоты,
    ...
    @profile("demo_gray_scott", "demo")
    def step_gray_scott(self, position=(0, 0)):
        """
    ...
    @profile("demo_ising_metropolis", "demo")
    def step_ising_metropolis(self, position=(0, 0)):
        """
        Реалистичная 2D-сцена: модель Изинга (ферромагнитная) на решётке с динамикой Метрополиса.
        Смысл: бинарные спины s = ±1 на узлах сетки, взаимодействие только с ближайшими соседями.
        Энергия (гамильтониан):
            H = -J Σ_{<ij>} s_i s_j - h Σ_i s_i
        Динамика: Метрополис — вероятностные перевороты спинов с вероятностью min(1, exp(-ΔE / T)).
        Визуализация: чёрно-белая карта спинов, HUD: T, магнитизация M, энергия E, acceptance rate.
        """
        import math
        import random
        ...
        # --- Инициализация один раз ---
        if not hasattr(self, "_ising_init"):
            self._ising_init = True

            # Параметры модели и сетки
            self._Nx = 50  # число узлов по X
            self._Ny = 50  # число узлов по Y
            self._J = 1.0  # обменный параметр J>0 => ферромагнитный
            self._h = 0.0  # внешнее поле h
            self._T = 2.5  # температура (k_B = 1)
            self._attempts_per_frame = self._Nx * self._Ny  # сколько попыток флипнуть за кадр
            self._dt = 1.0  # условное «время» за кадр (не физическая величина)

            # состояние: решётка спинов ±1
            Nx, Ny = self._Nx, self._Ny
            self._spins = [[1 if random.random() < 0.5 else -1 for _ in range(Ny)] for _ in range(Nx)]
            ...
            # для визуализации: размер клетки в пикселях
            self._grid_px = 10.0

        # --- Вспомогательные функции ---
        Nx, Ny = self._Nx, self._Ny
        J, h, T = self._J, self._h, self._T
        attempts_per_frame = self._attempts_per_frame
        grid_px = self._grid_px

        def idx(i, n):
            return (i + n) % n  # периодические ГУ

        # Локальный переход энергии при перевороте спина s -> -s:
        # ΔE = E_new - E_old = 2 * s * (J * sum_neighbors + h)
        def delta_energy(i, j):
            s = self._spins[i][j]
            nb = (self._spins[idx(i + 1, Nx)][j] +
                  self._spins[idx(i - 1, Nx)][j] +
                  self._spins[i][idx(j + 1, Ny)] +
                  self._spins[i][idx(j - 1, Ny)])
            return 2.0 * s * (J * nb + h)

        # --- Метрополис Monte-Carlo (несколько попыток на кадр) ---
        accepted = 0
        for _ in range(attempts_per_frame):
            ...
            dE = delta_energy(i, j)
            self._attempts += 1
            # принять всегда если dE <= 0, иначе с вероятностью exp(-dE/T)
            if dE <= 0.0 or random.random() < math.exp(-dE / max(T, 1e-12)):
                # flip
        ...
        # --- Диагностика: полная энергия и средняя магнитизация ---
        total_E = 0.0
        total_M = 0
        for i in range(Nx):
            for j in range(Ny):
                s = self._spins[i][j]
                # взаимодействие с двумя направлениями, чтобы не считать каждый сосед дважды:
                total_E += -J * s * (self._spins[idx(i + 1, Nx)][j] + self._spins[i][idx(j + 1, Ny)]) - h * s
                total_M += s
        # нормируем энергию на число узлов
        E_per_site = total_E / (Nx * Ny)
        M_per_site = total_M / (Nx * Ny)
        acceptance_rate = accepted / attempts_per_frame if attempts_per_frame > 0 else 0.0
        ...
        for i in range(Nx):
            for j in range(Ny):
                s = self._spins[i][j]
                # +1 -> тёплый оттенок (ближе к белому), -1 -> холодный (чёрный)
                if s > 0:
                    color = (230, 230, 220)
                else:
                    color = (20, 20, 30)
        ...
        # HUD: основные параметры и диагностика
        Gizmos.draw_text((cx - halfx + 8, cy - halfy - 20),
                         f"T={T:.3f}, J={J:.2f}, h={h:.2f}, attempts/frame={attempts_per_frame}",
                         font_size=14, color=(240, 240, 240), world_space=True)
        Gizmos.draw_text((cx - halfx + 8, cy - halfy - 36),
                         f"t={self._t:.1f}, M={M_per_site:.3f}, E={E_per_site:.3f}, acc_rate={acceptance_rate:.3f}",
                         font_size=14, color=(200, 200, 200), world_space=True)

        Gizmos.draw_text(
            (cx - halfx + 1400, cy - halfy + 340),
            """Что это такое (модель Изинга + Метрополис, в терминах этой реализации)
    Модель Изинга — простая модель ферромагнитного спинового массива: в каждой ячейке сетки находится спин s_i = ±1. Энергия системы
        H = -J Σ_{<ij>} s_i s_j - h Σ_i s_i
    где J — обменный интеграл (J>0 даёт тяготение к параллельным спинам, т.е. ферромагнетизм), h — внешнее магнитное поле. Здесь учтены только ближайшие соседи.

    Зачем это нужно (цели и применения)
    • Фазовые переходы: это минимальная модель, демонстрирующая переход порядка (магнитного) при изменении температуры: ниже T_c — большая магнитизация, выше — нулевая в среднем.  
    • Статистическая физика и методы Монте-Карло: Метрополис — базовый алгоритм для имитации распределений Больцмана.  
    • Обучение и визуализация: наглядный пример, показывающий как локальные стохастические правила приводят к глобальным эффектам (кластерам одинаковых спинов).  
    • Тестирование ускорений и алгоритмических приёмов (cluster updates, Wolff, Swendsen–Wang).

    Ключевые элементы и их роль (с точки зрения реализации)
    1. Решётка Nx×Ny — дискретизация пространства. Периодические ГУ (тор) убирают граничные эффекты.  
    2. Параметры J, h, T — определяют динамику: T управляет шумом (термальными флуктуациями), J — силу упорядоченности.  
    3. Метрополис: предложить локальный переворот спина и принять его с вероятностью min(1, exp(-ΔE/T)). Это гарантирует выбор конфигураций с весом пропорциональным exp(-H/T) в стационарном состоянии.  
    4. ΔE (локальная разность энергии): для переворота одного спина s -> -s выражается как 2 s (J*Σ_соседей + h).  
    5. Диагностика: средняя магнитизация M и энергия E контролируют состояние системы и помогают обнаружить фазовый переход.

    Как это работает пошагово (алгоритм за кадр)
    0. Инициализация: случайная или упорядоченная расстановка спинов; задаются J, h, T.  
    1. Повторить N_attempts (обычно ≈ Nx*Ny) раз: выбрать случайный узел (i,j), вычислить ΔE для возможного переворота, принять переворот с вероятностью min(1, e^{-ΔE/T}).  
    2. Обновить счётчики принятых переворотов и общее «время» t.  
    3. Посчитать глобальные величины: суммарная энергия H (взяв care чтобы не считать связи дважды) и средняя магнитизация M.  
    4. Визуализировать: +1 и −1 как два цвета, HUD с T, M, E и acceptance rate.

    Практические замечания и ограничения
    • Сходимость и время перемешивания: при температуре около критической T_c алгоритм перемешивается медленно (критическое замедление). Для больших сеток нужны ускоренные методы (кластерные алгоритмы).  
    • Размер сети: чтобы увидеть явный фазовый переход полезно иметь большие Nx×Ny (но это дороже вычислительно).  
    • Параметры: при J=1 и h=0 критическая температура для 2D квадратной решётки известна (~2.269), так что выбирайте T вокруг этого числа чтобы увидеть переход.  
    • Мониторинг acceptance rate: устойчивый диапазон ~0.2–0.6 удобен — слишком маленькая скорость принятия означает застревание, слишком большая — почти случайные перевороты.

    Интерпретация результатов
    • При T << T_c система быстро образует крупные домены одинаковой ориентации (|M|≈1).  
    • При T >> T_c домены мелкие и M≈0.  
    • Вблизи T_c вы увидите фрактоподобные кластерные структуры и сильные флуктуации M и E.

    Коротко: модель Изинга с Метрополисом — красивый, простой и мощный инструмент для изучения статистической механики, фазовых переходов и стохастической динамики. """,
            font_size=16,
            color='white',
            world_space=True,
            font_world_space=True,
    ...
    @profile("demo_nbody_3d_perspective", "demo")
    def step_nbody_3d_perspective(self, position=(0, 0)):
        """
        Реалистичная 2D-сцена: N-тел в трёхмерном пространстве с ньютоновским притяжением,
        интегрированные симплектически (leapfrog / kick-drift-kick), рендерятся проекцией
        на 2D плоскость с перспективой, глубинным затемнением и короткими следами траекторий.
        """
        import math
        import random
        ...
        if not hasattr(self, "_nbody3_init"):
            self._nbody3_init = True

            # физические параметры и интегратор
            self._N = 200  # количество тел
            self._G = 0.1  # гравитационная постоянная (в условных единицах)
            self._eps = 0.5  # softening длина для избежания сингулярности
            self._dt = 0.02  # шаг времени
            self._trail_len = 1  # длина следа (число старых позиций для отрисовки)
            self._merge_radius = 0.2  # порог слияния (опц.) — при слишком близком сближении сливаем тела

            # камера для перспективы: расположение (cam_x, cam_y, cam_z), фокусное расстояние f
            self._cam = (0.0, 0.0, 50.0)
            self._f = 180.0  # focal length (чем больше — слабее перспективное искажение)

            # состояния: массивы позиций (x,y,z), скоростей (vx,vy,vz), массы m
            N = self._N
            self._pos = [[0.0, 0.0, 0.0] for _ in range(N)]
            self._vel = [[0.0, 0.0, 0.0] for _ in range(N)]
            self._m = [1.0 for _ in range(N)]
            self._trail = [[] for _ in range(N)]  # список последних позиций для визуализации следов

            # Инициализация: центр + диск/сфера с вращением для динамичных структур
            for i in range(N):
                # радиусная упаковка: распределение по диску/слою сферического облака
                r = 2.0 + 8.0 * (random.random() ** 0.8)  # радиус
                theta = random.random() * 2.0 * math.pi
                phi = (random.random() - 0.5) * 0.8  # немного угла от плоскости
                x = r * math.cos(theta) * math.cos(phi)
                y = r * math.sin(theta) * math.cos(phi)
                z = r * math.sin(phi) + (random.random() - 0.5) * 0.5
                self._pos[i] = [x, y, z]

                # задать орбитальную скорость вокруг центра с некоторым вращением + шум
                # v ~ sqrt(G*M_enc/r) приблизительно; возьмём M_enc ~ N*mean_mass*0.5
                M_enc = N * 0.7
                v_mag = math.sqrt(max(1e-6, self._G * M_enc / (r + 0.1))) * 0.6
                ...
                # разнообразие масс
                self._m[i] = 0.6 + random.random() * 1.6

                # инициализируем трек
                self._trail[i] = [list(self._pos[i])]

            # сдвинем центр масс в начало координат и уберём суммарный импульс
            CMx = sum(self._m[i] * self._pos[i][0] for i in range(N)) / sum(self._m)
            CMy = sum(self._m[i] * self._pos[i][1] for i in range(N)) / sum(self._m)
            CMz = sum(self._m[i] * self._pos[i][2] for i in range(N)) / sum(self._m)
            Px = sum(self._m[i] * self._vel[i][0] for i in range(N))
            Py = sum(self._m[i] * self._vel[i][1] for i in range(N))
            Pz = sum(self._m[i] * self._vel[i][2] for i in range(N))
            for i in range(N):
                self._pos[i][0] -= CMx
                self._pos[i][1] -= CMy
                self._pos[i][2] -= CMz
                self._vel[i][0] -= Px / sum(self._m)
                self._vel[i][1] -= Py / sum(self._m)
                self._vel[i][2] -= Pz / sum(self._m)
        ...
        merge_r = self._merge_radius
        camx, camy, camz = self._cam
        f = self._f

        def compute_accelerations(positions, masses):
            # возвращаем список ускорений a[i] = [ax, ay, az]
            a = [[0.0, 0.0, 0.0] for _ in range(len(positions))]
            n = len(positions)
            for i in range(n):
        ...
        # --- Leapfrog (kick-drift-kick) интегратор с возможным слиянием тел ---
        # 1) полушаг velocity kick: v += 0.5*dt*a(t)
        a = compute_accelerations(self._pos, self._m)
        for i in range(len(self._pos)):
            self._vel[i][0] += 0.5 * dt * a[i][0]
            self._vel[i][1] += 0.5 * dt * a[i][1]
            self._vel[i][2] += 0.5 * dt * a[i][2]

        # 2) drift: x += dt * v(t+dt/2)
        for i in range(len(self._pos)):
            self._pos[i][0] += dt * self._vel[i][0]
            self._pos[i][1] += dt * self._vel[i][1]
            self._pos[i][2] += dt * self._vel[i][2]

        # 3) compute new accelerations at x(t+dt)
        a_new = compute_accelerations(self._pos, self._m)

        # 4) final half-kick: v += 0.5*dt*a(t+dt)
        for i in range(len(self._pos)):
            self._vel[i][0] += 0.5 * dt * a_new[i][0]
            self._vel[i][1] += 0.5 * dt * a_new[i][1]
            self._vel[i][2] += 0.5 * dt * a_new[i][2]

        # --- Простое слияние тел при очень близких сближениях: объединяем массы и сохраняем импульс ---
        # (уменьшает взрывное поведение при сильном взаимодействии и даёт интересную динамику агрегации)
        alive = [True] * len(self._pos)
        for i in range(len(self._pos)):
            if not alive[i]: continue
            xi, yi, zi = self._pos[i]
            for j in range(i + 1, len(self._pos)):
                if not alive[j]: continue
                xj, yj, zj = self._pos[j]
                dx = xj - xi
                dy = yj - yi
                dz = zj - zi
                if dx * dx + dy * dy + dz * dz < (merge_r * merge_r):
                    # merge j into i
                    mi = self._m[i]
                    mj = self._m[j]
                    totm = mi + mj
                    # conserve momentum: v_new = (mi*vi + mj*vj)/totm
                    for k in range(3):
                        self._vel[i][k] = (mi * self._vel[i][k] + mj * self._vel[j][k]) / totm
                    # position to center of mass
                    self._pos[i][0] = (mi * self._pos[i][0] + mj * self._pos[j][0]) / totm
                    self._pos[i][1] = (mi * self._pos[i][1] + mj * self._pos[j][1]) / totm
                    self._pos[i][2] = (mi * self._pos[i][2] + mj * self._pos[j][2]) / totm
                    self._m[i] = totm
                    alive[j] = False

        # compactify arrays to remove merged bodies
        if not all(alive):
            new_pos = []
            new_vel = []
            new_m = []
            ...
            for i in range(len(self._pos)):
                if alive[i]:
                    new_pos.append(self._pos[i])
                    new_vel.append(self._vel[i])
                    new_m.append(self._m[i])
                    new_trail.append(self._trail[i])
            self._pos = new_pos
            self._vel = new_vel
            self._m = new_m
        ...
        # --- Обновление следов (храним последние trail_len позиций) ---
        for i in range(len(self._pos)):
            self._trail[i].append(list(self._pos[i]))
            if len(self._trail[i]) > trail_len:
                self._trail[i].pop(0)

        self._t += dt

        # --- Рендеринг: проекция 3D -> 2D с перспективой и глубинной сортировкой ---
        # Проекционная функция: камера расположена в (camx, camy, camz), мы смотрим в -z направление.
        def project_point(p):
            x, y, z = p
            # переводим в координаты камеры
            zx = x - camx
            zy = y - camy
            zz = z - camz  # обычно отрицательное, чем ближе к камере — меньше по абсолютной
            # если точка позади камеры (zz >= 0), мы всё равно проектируем с "за камерой" поправкой
            # перспективная проекция: screen = f * (x_cam / -z_cam), -z_cam потому что камера смотрит в -z
            view_z = -zz if zz < 0 else -zz - 1e-3
            sx = cx + f * (zx / view_z)
            sy = cy + f * (zy / view_z)
            # глубина для сортировки и shading: используем view_z (положительная для объектов перед камерой)
            depth = view_z
            # scale для размера точки (далее домножим на массу^(1/3))
            scale = max(0.5, 6.0 / (0.5 + depth * 0.08))
            return (sx, sy, depth, scale)

        # Сформируем список рендер-объектов (один per body)
        render_list = []
        for i in range(len(self._pos)):
            sx, sy, depth, base_scale = project_point(self._pos[i])
            mass = self._m[i]
            size = base_scale * (mass ** (1.0 / 3.0))
            # brightness: ближе -> ярче
            brightness = max(0.12, min(1.0, 1.2 - depth * 0.02))
            render_list.append({
                "i": i, "sx": sx, "sy": sy, "depth": depth,
                "size": size, "brightness": brightness
            })

        # Сортируем по глубине: дальние объекты рисуем первыми
        render_list.sort(key=lambda r: r["depth"], reverse=True)

        # Фон / рамка области рендера
        view_w = 500
        view_h = 500
        Gizmos.draw_rect((cx, cy), view_w, view_h, color=(18, 20, 25), filled=True, world_space=True)
        Gizmos.draw_rect((cx, cy), view_w, view_h, color=(80, 80, 90), filled=False, thickness=2, world_space=True)

        # Рисуем следы (полупрозрачные) и сами тела (круги с затемнением по глубине)
        for item in render_list:
            ...
            size = item["size"]
            b = item["brightness"]
            # цвет тела: теплый центральный оттенок, затемнённый по глубине
            col = (int(220 * b), int(160 * b), int(90 * b))

            # нарисовать трек: соединяем несколько последних проекций (если есть)
            if len(self._trail[i]) >= 2:
                pts = []
                for p in self._trail[i]:
                    px, py, pdepth, pscale = project_point(p)
                    pts.append((px, py))
                # рисуем сегменты следа с постепенно уменьшающимся альфа (симулируем прозрачность)
                for k in range(len(pts) - 1):
                    a = 0.08 + 0.9 * (k / max(1, len(pts) - 2))
                    # визуальное упрощение: модуль цвета * alpha приближённо
                    c = (int(col[0] * a), int(col[1] * a), int(col[2] * a))
                    Gizmos.draw_line(pts[k], pts[k + 1], color=c, thickness=1, world_space=True)

            # рисуем тело (используем квадрат как запасную форму, если draw_circle недоступна)
            # предположим есть функция draw_circle; если нет, используем draw_rect с размерами size
            try:
                Gizmos.draw_circle((sx, sy), size, color=col, filled=True, world_space=True)
            except Exception:
                Gizmos.draw_rect((sx, sy), size * 1.2, size * 1.2, color=col, filled=True, world_space=True)

        # HUD: параметры и диагностическая информация
        total_mass = sum(self._m)
        KE = 0.0
        PE = 0.0
        for i in range(len(self._pos)):
            vx, vy, vz = self._vel[i]
            KE += 0.5 * self._m[i] * (vx * vx + vy * vy + vz * vz)
        # грубая оценка потенциальной энергии (парные суммы)
        for i in range(len(self._pos)):
            xi, yi, zi = self._pos[i]
            for j in range(i + 1, len(self._pos)):
                xj, yj, zj = self._pos[j]
                ...
                PE += -G * self._m[i] * self._m[j] / r
        E_tot = KE + PE

        Gizmos.draw_text((cx - view_w / 2 + 12, cy - view_h / 2 - 18),
                         f"t={self._t:.2f}, N={len(self._pos)}, M_tot={total_mass:.2f}",
                         font_size=14, color=(230, 230, 230), world_space=True)
        Gizmos.draw_text((cx - view_w / 2 + 12, cy - view_h / 2 - 36),
                         f"KE={KE:.3f}, PE={PE:.3f}, E={E_tot:.3f}, dt={dt}",
                         font_size=12, color=(200, 200, 200), world_space=True)

        # --- Подробное объяснение модели и рендера (в world_space) ---
        Gizmos.draw_text(
            (cx - view_w / 2 + 12, cy - view_h / 2 - 120),
            """Что это такое (N-тел в 3D с перспективной проекцией)
    Это простая N-тел симуляция в трёхмерном евклидовом пространстве с ньютоновским притяжением:
        a_i = G Σ_{j≠i} m_j (r_j - r_i) / (|r_j - r_i|^2 + ε^2)^(3/2)
    Интегратор — симплектический leapfrog (kick-drift-kick), который хорошо сохраняет энергию на длительных интервалах. Для численной устойчивости добавлена softening-длина ε, чтобы избежать бесконечных ускорений при близких подходах; при очень близких сближениях тела могут опционально сливаться (merging).

    Как устроен 3D → 2D рендер (в этой реализации)
    • Камера задана в точке (camx, camy, camz) и смотрит вдоль -z.  
    • Перспективная проекция: экранные координаты sx,sy = (cx,cy) + f * ( (x-camx) / view_z , (y-camy) / view_z ), где view_z = - (z - camz) для объектов перед камерой.  
    • Размер точки (и толщина следа) масштабируется обратно пропорционально depth (чем ближе — тем больше). Масса влияет на размер через правило size ∝ mass^(1/3).  
    • Глубинное затенение: яркость тела уменьшается с увеличением глубины (более дальние объекты тусклее).  
    • Траектории: храним небольшой буфер последних позиций и рисуем полупрозрачные линии, показывающие недавнюю историю движения.  
    • Сортировка по глубине: дальние объекты рисуются первыми, ближние — позже, чтобы естественно "перекрывать" дальние.

    Зачем это полезно (цели и применения)
    • Визуализация динамики гравитационных систем (скопления, аккреционные структуры, агрегация).  
    • Демонстрация симплектических интеграторов и их поведения в задачах с долгой эволюцией.  
    • Художественные и образовательные демонстрации: красивые спирали, сливающиеся ядра, динамические перестройки.  
    • Простая платформа для добавления новых эффектов: фрикция, внешние потенциалы, ротация камеры, управление параметрами в реальном времени.

    Практические замечания и ограничения
    • Численная сложность: ускорение O(N^2) — при N≳200 симуляция становится тяжёлой; возможные улучшения: Barnes–Hut, fast multipole.  
    • Softening ε — параметры чувствительны: слишком маленькое ε → резкие столкновения; слишком большое → "размазывание" гравитации.  
    • Слияния упрощают физику (не сохраняют энергию точно), но дают стабильный визуальный результат.  
    • Камера/фокус можно менять для драматического эффекта — уменьшение f усиливает перспективу.

    Коротко: этот демо-скрипт сочетает физически мотивированную 3D-динамику с простым, но выразительным 2D-проекционным рендером — результат красиво иллюстрирует эволюцию гравитационных систем и даёт удобный визуальный контроль над поведением.""",
            font_size=16,
            color='white',
            world_space=True,
            font_world_space=True,
