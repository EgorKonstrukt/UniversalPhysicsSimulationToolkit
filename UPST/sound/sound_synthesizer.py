...
class SoundSynthesizer:
    ...
    def __init__(self, sample_rate=config.synthesizer.sample_rate, buffer_size=config.synthesizer.buffer_size):
        if getattr(self, '_initialized', False):
            return
        self._initialize_mixer(sample_rate, buffer_size)
        self.sample_rate = sample_rate
        self.buffer_size = buffer_size
        self.volume = config.synthesizer.volume
        self.effects = {'vibrato_rate':5.0,'vibrato_depth':0.002,'tremolo_rate':5.0,'tremolo_depth':0.5,'reverb_amount':0.3,'delay_time':0.3,'delay_feedback':0.4,'chorus_rate':1.5,'chorus_depth':0.003,'distortion_amount':0.0}
        self.filter_settings = {'type':'lowpass','cutoff':2000,'resonance':1.0}
        self.lfo = {'rate':2.0,'depth':1.1,'target':'cutoff'}
        self.active_voices = []
        self._initialized = True
    def _initialize_mixer(self, sample_rate, buffer_size):
        if pygame.mixer.get_init():
            current_freq, _, current_channels = pygame.mixer.get_init()
    ...
    def _ensure_mixer_initialized(self):
        if not pygame.mixer.get_init():
            self._initialize_mixer(self.sample_rate, self.buffer_size)
        else:
            current_freq, _, current_channels = pygame.mixer.get_init()
            if current_channels != 2 or current_freq != self.sample_rate:
                self._initialize_mixer(self.sample_rate, self.buffer_size)
