...
class ScriptEditorWindow:
    def __init__(self, rect, manager, physics_manager, physics_debug_manager=None, owner=None, script=None, app=None):
        ...
        self.script = script
        self.file_path = None
        self.window = pygame_gui.elements.UIWindow(rect=rect, manager=self.manager,
                                                   window_display_title=self._get_title(),
                                                   object_id=ObjectID(class_id='@script_editor_window'), resizable=True,
                                                   draggable=True)
        self._create_ui()
        ...
        self._syntax_colors = {'keyword': (255, 100, 100), 'string': (100, 255, 100), 'comment': (150, 150, 150),
                               'number': (100, 100, 255), 'default': (220, 220, 220)}
    ...
    def _create_elements(self):
        ...
        for i, (text, cb, x_norm, obj_id, width_norm) in enumerate(btn_defs):
            ...
            btn = UIButton(relative_rect=pygame.Rect(x, y, width, self.layout['btn_h']), text=text,
    ...
    def load_script_from_file(self):
        ...
        try:
            ...
            self.show_error("Load Error", f"Failed to load script:<br>{str(e)}")

    def save_script_to_file(self):
        init_dir = os.path.dirname(self.file_path) if self.file_path else os.getcwd()
        root = tk.Tk()
        root.withdraw()
        path = filedialog.asksaveasfilename(title="Save Python Script", initialdir=init_dir,
                                            initialfile=self.name_input.get_text() or "script.py",
                                            defaultextension=".py",
                                            filetypes=[("Python files", "*.py"), ("All files", "*.*")])
        if not path: return
        try:
            with open(path, 'w', encoding='utf-8') as f:
                f.write(self.code_box.get_text())
            self.file_path = path
            self.update_status(f"Saved: {os.path.basename(path)}")
        except Exception as e:
            self.show_error("Save Error", f"Failed to save script:<br>{str(e)}")

    def show_error(self, title, msg):
        UIMessageWindow(rect=pygame.Rect(200, 200, 350, 180), manager=self.manager, window_title=title,
                        html_message=msg, object_id=ObjectID(class_id='@error_message'))
    ...
    def apply_script(self):
        ...
        try:
            ...
            self.show_error("Syntax Error", f"Line {se.lineno}: {se.msg}"); return
        except Exception as e:
            self.show_error("Compile Error", f"Compile failed:<br>{str(e)}"); return
        try:
            target = self.target_dropdown.selected_option
            owner = None
            if target != "World": owner = self.owner if self.owner is not None else (
                self.physics_debug_manager.selected_body if self.physics_debug_manager else None)
    ...
    def _highlight_syntax(self):
        ...
        for tok_type, tok_str, (srow, scol), (erow, ecol) in tokens:
            ...
            color_type = None
            if tok_type == tokenize.NAME and keyword.iskeyword(tok_str):
                color_type = 'keyword'
    ...
    def process_event(self, event):
        ...
        if event.type == pygame.WINDOWRESIZED:
            ...
            btn_map = {self.btn_0: self.load_btn_cb, self.btn_1: self.insert_stub_cb, self.btn_2: self.save_btn_cb,
                       self.btn_3: self.cancel_btn_cb, self.btn_4: self.apply_btn_cb}
