        ...
        if self.threaded:
            ...
            if self._start_fn:
                try:
                    ...
                    Debug.log_exception(f"Error in script '{self.name}' start(): {e}", "Scripting")
    ...
    def update(self, dt: float = 0.0):
        ...
        if self._update_fn:
            try:
                ...
                Debug.log_exception(f"Error in script '{self.name}' update(): {e}", "Scripting")
    ...
    def stop(self):
        ...
        if not self.threaded and self._stop_fn:
            try:
                ...
                Debug.log_exception(f"Error in script '{self.name}' stop(): {e}", "Scripting")
    ...
    def _threaded_loop(self):
        if self._start_fn:
            try:
                ...
                Debug.log_exception(f"Error in threaded script '{self.name}' start(): {e}", "Scripting")
        while self.running:
            if self._update_fn:
                try:
                    self._update_fn(1.0 / 100.0)
                except Exception as e:
                    Debug.log_exception(f"Error in threaded script '{self.name}' update(): {e}", "Scripting")
            time.sleep(1.0 / 100.0)
        if self._stop_fn:
            try:
                self._stop_fn()
            except Exception as e:
                Debug.log_exception(f"Error in threaded script '{self.name}' stop(): {e}", "Scripting")